...
#add in DSR
HHmembers<-merge(HHmembers,DSRoverallhigh_summary, by="customer")
#omimt na values
HHmembers<-HHmembers%>%
  na.omit()
...
kids<-merge(kids,DSRoverallhigh_summary, by="customer")
kids<-kids%>%
  na.omit()
...
rooms<-merge(rooms,DSRoverallhigh_summary, by="customer")
rooms<-rooms%>%
  na.omit()
...
custom_colors <- c("#00bfff", "#3399ff","#0066cc")
